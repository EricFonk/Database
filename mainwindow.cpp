...
{
	if (schemaTree->currentItem() != NULL)
	{
		//判断是否选中根节点 弹出数据库右键菜单
...
void MainWindow::selectAllofOneTable()
{
	...
	double timestamp;
	long affectedRows;
	resflag = executeWithQuery(1, "use " + schemaTree->currentItem()->parent()->parent()->text(0) + ";", timestamp, affectedRows);
...
void MainWindow::createOneTable()
{
	...
	//connect(tableAlter, SIGNAL(createTableSuccess(QString)), this, SLOT(addTableToTree(QString)));
...
void MainWindow::manageTreeInit()
{
	...
	QTreeWidgetItem *temp_root4 = new QTreeWidgetItem(manageTree, QStringList("Export and Import"));
}
...
void MainWindow::initManageInfo(QTreeWidgetItem *temp_root, int temp)
{
	...
	else if (manageTree->currentItem()->text(0).compare("Export and Import") == 0)
	{
		QWidget *temp = rightLayout->itemAt(0)->widget();
		rightLayout->removeWidget(temp);
		delete(temp);
